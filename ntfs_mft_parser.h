...
public:
	...
	// this func is used to get differences b/w 2 MFT tables
	// it returns map, where each entry is either a file which is missing in another mft table (false value)
	// or a file which is present in another mft table (true value) but missing in current mft table
	std::map<uint64_t, change_type_n> diff(Ntfs_c& other);

	// In ntfs_mft_parser.h, update the function signature:
	std::pair<const uint64_t, Ntfs_c::FileRecord*>* FindRecord(std::pair<uint64_t, Ntfs_c::FileRecord*> rec, Ntfs_c& other);
	Ntfs_c::change_type_n detectRecordChange(Ntfs_c& prev_mft, Ntfs_c::FileRecord& prev_rec, Ntfs_c& cur_mft, Ntfs_c::FileRecord& cur_rec);
	static std::string ChangeType2Str(Ntfs_c::change_type_n change);
...
inline int64_t stringToInt64(const std::string& str) {
	try {
